...
<nav class="main_navigation  (isset($_COOKIE['menu_opened']) && $_COOKIE['menu_opened'] == 'true') ? 'opened no_animation' : '' ?>">
    <div class="employee_info">
        ...
        <span class="name"> session->u->profile->FirstName ?>  session->u->profile->LastName ?></span>
        <span class="privilege"> session->u->GroupName ?></span>
    </div>
    <ul class="app_navigation">
        <li class=" matchUrl('/') === true ? ' selected' : '' ?>"><a href="/"><i class="fa fa-dashboard"></i>  general_statistics ?></a></li>
        <li class="submenu">
        ...
        <li class=" matchUrl('/suppliers') === true ? ' selected' : '' ?>"><a href="/suppliers"><i class="material-icons">group</i>  suppliers ?></a></li>
        <li class="submenu">
